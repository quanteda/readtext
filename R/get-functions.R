...
get_json <- function(path, textfield, encoding, ...) {
    ...
    error = function(e) {
        tryCatch({
            if (verbosity >= 1) warning("Doesn't look like Tweets json file, trying general JSON")
        ...
        error = function(e) {
            ...
            if (verbosity >= 1) warning("File doesn't contain a single valid JSON object, trying line-delimited json")
