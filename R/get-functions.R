...
get_json <- function(path, textfield, encoding, ...) {
    ...
    error = function(e) {
        tryCatch({
            if (options('readtext-verbosity') >= 1) warning("Doesn't look like Tweets json file, trying general JSON")
        ...
        error = function(e) {
            ...
            if (options('readtext-verbosity') >= 1) warning("File doesn't contain a single valid JSON object, trying line-delimited json")
...
get_xml <- function(path, textfield, encoding,...) {
    ...
    if (is_probably_xpath(textfield))  {
        ...
        if (is.character(textfield)) {
            ...
            if (options('readtext-verbosity') >= 1) {
